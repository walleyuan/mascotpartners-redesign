...
if ( ! class_exists( 'RH_Admin_Menu' ) ) :
	...
	class RH_Admin_Menu {
		...
		public function open_menu() {
			...
			$menu_arr = apply_filters( 'real_homes_open_menus_slugs', [
				'property',
				'edit-property',
			...
			if ( in_array( $screen->id, $menu_arr ) ) { ?>
				<script type="text/javascript">
					...
					if ( isset( $get_array['taxonomy'] ) && ( 'property-feature' === $get_array['taxonomy'] ) ) {
						echo 'jQuery("#toplevel_page_real_homes ul li:nth-child(4)").addClass("current");';
					}
					if ( isset( $get_array['taxonomy'] ) && ( 'property-type' === $get_array['taxonomy'] ) ) {
						echo 'jQuery("#toplevel_page_real_homes ul li:nth-child(5)").addClass("current");';
					}
					if ( isset( $get_array['taxonomy'] ) && ( 'property-city' === $get_array['taxonomy'] ) ) {
						echo 'jQuery("#toplevel_page_real_homes ul li:nth-child(6)").addClass("current");';
					}
					if ( isset( $get_array['taxonomy'] ) && ( 'property-status' === $get_array['taxonomy'] ) ) {
						echo 'jQuery("#toplevel_page_real_homes ul li:nth-child(7)").addClass("current");';
					}
					// if ( isset( $get_array['page'] ) && ( 'properties-payments' === $get_array['page'] ) ) {
					// 	echo 'jQuery("#toplevel_page_real_homes ul li:nth-child(8)").addClass("current");';
					// }
					if ( isset( $get_array['post_type'] ) && ( 'agent' === $get_array['post_type'] ) ) {
						echo 'jQuery("#toplevel_page_real_homes ul li:nth-child(8)").addClass("current");';
					}
					if ( isset( $get_array['post_type'] ) && ( 'partners' === $get_array['post_type'] ) ) {
						echo 'jQuery("#toplevel_page_real_homes ul li:nth-child(9)").addClass("current");';
					}
					if ( isset( $get_array['post_type'] ) && ( 'slide' === $get_array['post_type'] ) ) {
						echo 'jQuery("#toplevel_page_real_homes ul li:nth-child(10)").addClass("current");';
					}
					if ( isset( $get_array['page'] ) && ( 'inspiry-real-homes-design' === $get_array['page'] ) ) {
						echo 'jQuery("#toplevel_page_real_homes ul li:nth-child(12)").addClass("current");';
					}
					if ( isset( $get_array['page'] ) && ( 'pt-one-click-demo-import' === $get_array['page'] ) ) {
						echo 'jQuery("#toplevel_page_real_homes ul li:nth-child(13)").addClass("current");';
					}
					?>
				</script>
				<?php
